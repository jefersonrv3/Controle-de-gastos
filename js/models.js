...
/* ------------------------------------------------------------
   DINHEIRO A RECEBER (recebível)
   { id, descricao, valorTotal, numParcelas, primeiroVencimento,
     parcelasPagas, contaDestinoId }

   Repare que o formato é IDÊNTICO ao da Dívida (mesmos nomes de campo:
   valorTotal, numParcelas, primeiroVencimento, parcelasPagas). Isso é
   proposital: significa que calcularValorParcela(), calcularParcelasRestantes(),
   calcularProximoVencimento() e dividaQuitada() — já escritas mais abaixo —
   funcionam SEM NENHUMA ALTERAÇÃO também pra recebível, porque elas só leem
   esses 4 campos, não importa se o objeto é uma "dívida" ou um "recebível".
   Isso é reaproveitamento de código: mesma lógica, dois contextos diferentes.

   contaDestinoId: definido na criação, é pra ONDE o dinheiro vai quando
   uma parcela for marcada como recebida (diferente da dívida, que não
   precisa saber de conta nenhuma).
   ------------------------------------------------------------ */
function criarRecebivel(descricao, valorTotal, numParcelas, primeiroVencimento, contaDestinoId) {
  return {
    id: gerarId(),
    descricao: descricao,
    valorTotal: Number(valorTotal),
    ...
    parcelasPagas: [], // aqui "paga" = "recebida"; mesmo nome de campo por reaproveitar a lógica
    contaDestinoId: contaDestinoId,
  };
}

/* ------------------------------------------------------------
   REGRAS DE CÁLCULO DE DÍVIDA (e também usadas por recebível — ver acima)
   Centralizar esses cálculos aqui (em vez de espalhar contas de
   data pelo ui.js) é o que torna fácil de testar e de explicar:
   "toda a matemática da dívida mora nessas 4 funções".
   ------------------------------------------------------------ */
